...
# injecte celles de ``Settings`` (config.toml, ``PURIDENTITYSERVER_*``, ``.env``).
...
def configure_identity(
    *,
    ...
    hiérarchie config.toml → `PURIDENTITYSERVER_*` → ``.env`` permet de remplacer
...
async def seed_users(seed: Mapping[str, Mapping[str, str]]) -> dict[str, User]:
    ...
    (config.toml / `PURIDENTITYSERVER_IDENTITY_SEED_USERS` / ``.env``). Les comptes
...
<head>
  <meta charset="utf-8">
  <title>PurIdentityServer — connexion</title>
  <style>
...
<body>
  <h1>Connexion à PurIdentityServer</h1>
