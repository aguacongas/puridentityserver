...
| `PURIDENTITYSERVER_IDENTITY_RESOURCES_SEED` | *(config.toml)* | Seed des IdentityResources (scopes identité, OIDC Core 1.0 §5.4) : liste JSON de resources (format `[{"name":"profile","display_name":"Votre profil","user_claims":["name","family_name"],"show_in_discovery_document":true}]`). Chaque resource déclare un scope et les claims exposés quand ce scope est accordé au jeton ; `show_in_discovery_document` (facultatif, défaut `true`) contrôle sa présence dans `scopes_supported`. Elles alimentent `scopes_supported` / `claims_supported` du discovery et le filtrage des claims de `/userinfo` par scope accordé ; aucune resource listée = application des ressources standard (openid, profile, email, address, phone, offline_access). Gestion en cours de vie via l'API CRUD `GET/POST /identity-resources` et `GET/PUT/DELETE /identity-resources/{name}`. |
