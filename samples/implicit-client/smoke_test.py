"""Smoke test du flow Implicit (`samples/implicit-client`).
...
2. `/login` redirige le navigateur vers la page de login du serveur
   avec ``response_type=id_token token`` (et ``nonce``, sans
   ``code_challenge``) ;
3. le login réel du serveur (`/login`, cookie de session) est validé et
   redirige vers l'URL `/authorize` construite par le client ;
4. le serveur remet ``id_token`` + ``access_token`` **dans le fragment**
   de l'URL de redirection (query string vide, RFC 6749 §4.2.2) ;
5. le client reçoit les jetons du fragment (`/collect`), vérifie
   l'``id_token`` (JWKS, ``aud``, ``nonce``) et le lien ``at_hash``,
   puis interroge ``/userinfo`` et affiche les claims ;
...
def _assert_login_redirect(authorize_url: str) -> tuple[str, str]:
    """Valide l'URL d'autorisation du flow Implicit et renvoie état + nonce."""
    params = parse_qs(urlparse(authorize_url).query)
    assert params["response_type"] == ["id_token token"]
    assert "code_challenge" not in params, "le flow Implicit n'utilise pas PKCE"
    assert params["client_id"] == ["sample-implicit-client"]
...
def _assert_fragment_redirect(location: str, state: str) -> dict[str, str]:
    """Valide la remise des jetons : query vide, jetons dans le fragment."""
    parsed = urlparse(location)
    assert not parsed.query, f"les jetons ne doivent pas être dans la query : {location}"
...
def _run_flow() -> None:
    """Exécute les huit étapes du flow Implicit de bout en bout (login + logout)."""
    with httpx.Client(follow_redirects=False, timeout=_HTTP_TIMEOUT) as http:
        ...
        print("  [4/8] authorize OK (id_token + access_token remis dans le fragment)")
        ...
        print(
            "  [5/8] collect OK (id_token vérifié + at_hash contrôlé + "
            f"userinfo, nonce={nonce[:8]}...)"
        )
        ...
        assert logout_params["post_logout_redirect_uri"] == ["http://127.0.0.1:5175/post-logout"]
        ...
        assert response.status_code == 302, response.text
        assert response.headers["location"].startswith("http://127.0.0.1:5175/post-logout"), (
...
def main() -> None:
    ...
    try:
        ...
        _client_proc = subprocess.Popen(
            [sys.executable, "samples/implicit-client/app.py"],
            cwd=REPO_ROOT,
            env=env,
        )
